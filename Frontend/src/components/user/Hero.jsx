      ...
      <div className="relative max-w-7xl w-full grid grid-cols-1 md:grid-cols-[0.45fr_0.55fr] items-center gap-8">
        ...
        >
          ...
          <motion.a
            ...
            whileTap={{ scale: 0.95 }}
            className="px-12 py-4 bg-green-600 text-white text-lg font-semibold rounded-full shadow-lg tracking-wide transition-all duration-300"
          >
            Explore Products
          </motion.a>
        </motion.div>
        ...
        >
          <motion.img
            src={assets.girl}
            alt="Hero Model"
            className="w-full max-w-[1250px] h-auto object-contain drop-shadow-2xl brightness-110 contrast-110
                       scale-[1.15] md:scale-[1.22] lg:scale-[1.26]
      ...
      {show && (
        <motion.div
          ...
          className={`
            absolute
            bg-white w-[300px]
            rounded-2xl shadow-2xl border border-gray-200 p-4 flex gap-4 z-50
            top-[50%] -translate-y-1/2

            ${cardSide === "left" ? "-left-80" : "left-[55px]"}
          `}
        >
          {/* ARROW */}
          {cardSide === "left" ? (
            <div
              className="absolute top-1/2 -translate-y-1/2 -right-3
                            w-0 h-0 border-t-10 border-b-10 border-l-12
                            border-t-transparent border-b-transparent border-l-white"
            />
          ) : (
            <div
              className="absolute top-1/2 -translate-y-1/2 -left-3
                            w-0 h-0 border-t-10 border-b-10 border-r-12
