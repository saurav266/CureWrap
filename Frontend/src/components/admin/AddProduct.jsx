...
export default function AddProduct() {
  ...
  const addImageField = () => {
    setProduct({
      ...product,
      images: [...product.images, { url: "", alt_text: "", is_primary: false }],
    });
  };

  const setPrimaryImage = (index) => {
    const updated = product.images.map((img, i) => ({ ...img, is_primary: i === index }));
    setProduct({ ...product, images: updated });
  };
  ...
  const addColorImage = (colorIndex) => {
    const updated = [...product.colors];
    updated[colorIndex].images.push({ url: "", alt_text: "", is_primary: false });
    setProduct({ ...product, colors: updated });
  };
  ...
  const addSize = (colorIndex) => {
    const updated = [...product.colors];
    updated[colorIndex].sizes.push({ size: "", price: "", sale_price: "", stock: "", quantity: 1 });
    setProduct({ ...product, colors: updated });
  };
    ...
    <div className="max-w-4xl mx-auto p-6 bg-white shadow rounded">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <div>
          ...
          {product.images.map((img, i) => (
            <div key={i} className="flex gap-2 mb-2 items-center">
              ...
              <input
                ...
                value={img.alt_text}
                onChange={(e) => handleImageChange(i, "alt_text", e.target.value)}
                className="w-full p-2 border rounded"
              />
              <button
                type="button"
                onClick={() => setPrimaryImage(i)}
                className={`px-2 py-1 rounded text-white ${img.is_primary ? "bg-green-600" : "bg-gray-400"}`}
              >
          ...
          <button type="button" onClick={addImageField} className="px-3 py-1 bg-blue-500 text-white rounded">
            + Add Image
          </button>
        </div>
        ...
        <div>
          ...
          {product.colors.map((color, ci) => (
            <div key={ci} className="border p-3 mb-3 rounded">
              <input
                ...
                placeholder="Color"
                value={color.color}
                onChange={(e) => updateColorField(ci, "color", e.target.value)}
                ...
                {color.images.map((img, ii) => (
                  <div key={ii} className="flex gap-2 mb-1 items-center">
                    <input
                      ...
                      placeholder="Image URL"
                      value={img.url}
                      onChange={(e) => updateColorImage(ci, ii, "url", e.target.value)}
                      className="w-full p-2 border rounded"
                    />
                    <input
                      type="text"
                      placeholder="Alt Text"
                      value={img.alt_text}
                      onChange={(e) => updateColorImage(ci, ii, "alt_text", e.target.value)}
                      className="w-full p-2 border rounded"
                    />
                    <button
                      type="button"
                ...
                {color.sizes.map((size, si) => (
                  <div key={si} className="border p-2 mb-2 rounded">
                    ...
                    <input
                      ...
                      placeholder="Price"
                      value={size.price}
                      onChange={(e) => updateSize(ci, si, "price", e.target.value)}
                      className="w-full p-2 border rounded mb-1"
                    />
                    <input
                      type="number"
                      ...
                      onChange={(e) => updateSize(ci, si, "sale_price", e.target.value)}
                      className="w-full p-2 border rounded mb-1"
                    />
                    <input
                      type="number"
                      placeholder="Stock"
                      value={size.stock}
                      onChange={(e) => updateSize(ci, si, "stock", e.target.value)}
                      className="w-full p-2 border rounded mb-1"
                    />
                    <button
                      type="button"
          ...
          <button type="button" onClick={addColor} className="px-3 py-1 bg-blue-700 text-white rounded">
            + Add Color
          </button>
        </div>
